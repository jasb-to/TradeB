...
// NEW: Silver session filter with extended hours option
const isSilverSessionAllowed = (timestamp: Date, extendedHours: boolean = false): boolean => {
  const date = new Date(timestamp)
  const utcHours = date.getUTCHours()
  const utcMinutes = date.getUTCMinutes()
  
  // Standard session: 07:00-17:00 UTC
  const timeInMinutes = utcHours * 60 + utcMinutes
  const startMinutes = 7 * 60 // 07:00 UTC
...
// NEW: Silver volatility filter with relaxed options
const isSilverVolatilityAdequate = (atr: number, relaxed: boolean = false): boolean => {
  if (relaxed) {
    return atr >= 0.15 // Relaxed: minimum ATR ≥ 0.15
  }
  return atr >= 0.25 // Standard: minimum ATR ≥ 0.25
}

// NEW: Silver MTF alignment with multiple options
const isSilverMTFAligned = (
  dailyBias: string, 
  h4Bias: string, 
  h1Bias: string,
  alignmentType: "ANY_TWO" | "ANY_ONE" | "RELAXED" = "ANY_TWO"
): boolean => {
  ...
  switch (alignmentType) {
    case "ANY_TWO":
      return dailyPlusFourH || fourHPlus1H
    case "ANY_ONE":
      return dailyPlusFourH || fourHPlus1H || dailyPlus1H
    case "RELAXED":
      // Allow single timeframe alignment with strong confirmation
      const hasStrongBias = [dailyBias, h4Bias, h1Bias].filter(b => b !== "NEUTRAL").length >= 1
      const hasConfirmation = h1Bias !== "NEUTRAL" // 1H must have clear bias
      return hasStrongBias && hasConfirmation
  }
}

// NEW: Evaluate Silver signal with multiple strategy variations
const evaluateSilverEntry = (
  dailyBias: string, 
  h4Bias: string, 
  h1Bias: string, 
  adx1h: number, 
  ...
  atr: number, 
  symbol: string,
  strategyType: "CONSERVATIVE" | "MODERATE" | "AGGRESSIVE" = "CONSERVATIVE"
): any => {
  // Strategy-specific parameters
  let sessionExtended = false
  let volatilityRelaxed = false
  let alignmentType: "ANY_TWO" | "ANY_ONE" | "RELAXED" = "ANY_TWO"
  let adxMin = 18
  let atrMin = 0.25
  ...
  switch (strategyType) {
    case "MODERATE":
      ...
      volatilityRelaxed = true
      alignmentType = "ANY_ONE"
      adxMin = 16
      atrMin = 0.15
      break
    case "AGGRESSIVE":
  ...
  // Session filter
  if (!isSilverSessionAllowed(new Date(), sessionExtended)) {
    return null
  }
  
  // Volatility filter
  if (!isSilverVolatilityAdequate(atr, volatilityRelaxed)) {
    return null
  }
  
  // MTF alignment based on strategy
  if (!isSilverMTFAligned(dailyBias, h4Bias, h1Bias, alignmentType)) {
    return null
  }
  
  // Setup quality with strategy-specific thresholds
  const alignedDirection = (dailyBias === h4Bias ? dailyBias : h4Bias) || "NONE"
  const isAPlusSetup = adx1h >= 22 && atr >= atrMin
  const isASetup = adx1h >= adxMin && atr >= atrMin
  ...
  const takeProfit = alignedDirection === "LONG" 
    ? currentPrice + atrPoints * 2.5 
    : currentPrice - atrPoints * 2.5
...
// UPDATED: Gold evaluation with Tier B support
const evaluateEntry = (
  dailyBias: string, 
  h4Bias: string, 
  h1Bias: string, 
  ...
  // NEW: Use updated setup tier logic with Tier B support
...
// NEW: Updated setup tier logic with Tier B support
...
const runBacktest = async (strategyType: "CONSERVATIVE" | "MODERATE" | "AGGRESSIVE" = "CONSERVATIVE"): Promise<BacktestResults> => {
  console.log(`[v0] Starting ${strategyType} Silver Strategy Backtest for XAU_USD and XAG_USD...`)
  console.log(`[v0] Strategy: ${strategyType}`)
  console.log("[v0] Fetching 90 days of historical data...\n")

  const results: BacktestResults = {
    XAU_USD: { trades: 0, wins: 0, losses: 0, profit: 0, tradeDetails: [], tierBreakdown: { A: 0, APlus: 0, B: 0 } },
    XAG_USD: { trades: 0, wins: 0, losses: 0, profit: 0, tradeDetails: [], tierBreakdown: { A: 0, APlus: 0, B: 0 } },
  }
  ...
  for (const symbol of ["XAU_USD", "XAG_USD"]) {
    ...
    for (let i = 20; i < minLength; i++) {
      ...
      // NEW: Use appropriate evaluation based on symbol
      if (symbol === "XAG_USD") {
        entry = evaluateSilverEntry(dailyBias, h4Bias, h1Bias, adx1h, currentPrice, atr, symbol, strategyType)
      } else {
        entry = evaluateEntry(dailyBias, h4Bias, h1Bias, m15Bias, m5Bias, adx1h, currentPrice, atr)
      }
...
const runAllStrategies = async (): Promise<void> => {
  console.log("=".repeat(160))
  console.log("SILVER STRATEGY OPTIMIZATION ANALYSIS")
  console.log("=".repeat(160))
  ...
  for (const strategy of strategies) {
    ...
    console.log("Symbol\t\tTrades\tWins\tLosses\tWin%\t\tProfit\t\tAvg/Trade\tA+\tA\tB")
    console.log("-".repeat(140))
    ...
    for (const symbol of ["XAU_USD", "XAG_USD"]) {
      ...
      console.log(
        `${symbol}\t\t${r.trades}\t${r.wins}\t${r.losses}\t${winPercent}%\t\t${r.profit.toFixed(2)}\t\t${avgTrade}\t\t${aplusTrades}\t${aTrades}\t${bTrades}`,
      )
    }
  ...
  console.log("\n[v0] Silver optimization analysis complete!")
}
