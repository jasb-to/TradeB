...
/**
 * Simple Backtest Script for Gold (XAU) and Silver (XAG)
...
class SimpleBacktest {
  ...
  async run(): Promise<void> {
    console.log(`\n🚀 Starting Simple Backtest for ${this.symbol}`)
    console.log(`📊 Starting Balance: £${this.startingBalance.toLocaleString()}`)
    
    try {
      const dataFetcher = new DataFetcher(this.symbol)
      ...
      const [dailyCandles, h4Candles, h1Candles, m15Candles, m5Candles] = await Promise.all([
        dataFetcher.fetchCandles("1d", 90),
        dataFetcher.fetchCandles("4h", 200),
        dataFetcher.fetchCandles("1h", 500),
        dataFetcher.fetchCandles("15m", 1000),
        dataFetcher.fetchCandles("5m", 1000),
      ])
      ...
      console.error(`❌ Backtest failed for ${this.symbol}:`, error)
    }
  }
  ...
  private async evaluateSignal(
    ...
    try {
      ...
      // Use TradingStrategies for both Gold and Silver
  ...
  private shouldTakeTrade(tier: "A+" | "A" | "B", signal: any): boolean {
  ...
  private calculatePositionSize(balance: number, riskPercent: number, signal: any): number {
  ...
  private checkExit(currentCandle: any, trade: Trade): { shouldExit: boolean; reason: string } {
  ...
  private findCandleIndex(candles: any[], targetTime: number): number {
  ...
  private printSummary(results: BacktestResult[]): void {
    console.log("\n" + "=".repeat(80))
    console.log(`📊 BACKTEST RESULTS - ${this.symbol}`)
...
async function runSimpleBacktest(): Promise<void> {
  console.log("🚀 SIMPLE GOLD & SILVER BACKTEST")
  ...
  for (const symbol of symbols) {
    ...
    const backtest = new SimpleBacktest(symbol, 10000)
    await backtest.run()
  }
