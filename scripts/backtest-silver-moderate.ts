...
const runBacktest = async (): Promise<BacktestResults> => {
  console.log("[v0] Starting MODERATE SILVER STRATEGY Backtest for XAU_USD and XAG_USD...")
  ...
  for (const symbol of ["XAU_USD", "XAG_USD"]) {
    ...
    for (let i = 20; i < minLength; i++) {
      ...
      // MODERATE: Use appropriate evaluation based on symbol
      if (symbol === "XAG_USD") {
...
const runModerateSilverBacktest = async (): Promise<void> => {
  console.log("=".repeat(160))
  console.log("MODERATE SILVER STRATEGY - PHASE 1 IMPLEMENTATION")
  console.log("=".repeat(160))
  ...
  console.log("\n[v0] Moderate Silver strategy backtest complete!")
}

runModerateSilverBacktest().catch(console.error)