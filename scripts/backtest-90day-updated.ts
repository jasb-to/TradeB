...
// NEW: Silver session filter
const isSilverSessionAllowed = (timestamp: Date): boolean => {
  const date = new Date(timestamp)
  const utcHours = date.getUTCHours()
  const utcMinutes = date.getUTCMinutes()
  
  // Silver trades only allowed 07:00-17:00 UTC
...
// NEW: Silver volatility filter
const isSilverVolatilityAdequate = (atr: number): boolean => {
  return atr >= 0.25
}

// NEW: Silver MTF alignment (ANY TWO timeframes)
...
// NEW: Evaluate Silver signal with strict requirements
const evaluateSilverEntry = (
  ...
  // Session filter
  if (!isSilverSessionAllowed(new Date())) {
    return null
  }
  
  // Volatility filter
  if (!isSilverVolatilityAdequate(atr)) {
    return null
  }
  
  // MTF alignment: ANY TWO timeframes must align
  if (!isSilverMTFAligned(dailyBias, h4Bias, h1Bias)) {
    return null
  }
  
  // Setup quality
  const alignedDirection = (dailyBias === h4Bias ? dailyBias : h4Bias) || "NONE"
  const isAPlusSetup = adx1h >= 22 && atr >= 0.25
  const isASetup = adx1h >= 18 && atr >= 0.25
  ...
  return {
    ...
    alignmentScore: 6, // Silver uses different scoring
    adx: adx1h,
  }
}
...
const runBacktest = async (): Promise<void> => {
  console.log("[v0] Starting UPDATED 90-day backtest for XAU_USD and XAG_USD...")
  console.log("[v0] Using current production strategy logic with Tier B support")
  ...
  for (const symbol of ["XAU_USD", "XAG_USD"]) {
    ...
    for (let i = 20; i < minLength; i++) {
      ...
      if (symbol === "XAG_USD") {
        entry = evaluateSilverEntry(dailyBias, h4Bias, h1Bias, adx1h, currentPrice, atr, symbol)
      } else {
  ...
  console.log("\n" + "=".repeat(140))
  console.log("UPDATED 90-DAY BACKTEST RESULTS")
